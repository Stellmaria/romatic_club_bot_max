...
async def auction_winner_loop(bot: Bot, discussion_chat_id: int):
    """Проверяет завершившиеся аукционы, объявляет победителя и завершает лот."""
    while True:
        try:
            now = utc_now()
            auctions = await list_auctions(["active"])
            for auction in auctions:
                if winner_deadline_reached(auction.get("end_time"), now=now):
                    bids = await get_bids_for_auction(auction["auction_id"])
                    await announce_winner(bot, auction, bids)
                    await update_auction_status(auction["auction_id"], "finished")
        except Exception as e:
            # логируем и едем дальше, чтобы цикл не падал
            logging.getLogger("auction").warning("winner_loop error: %s", e, exc_info=True)
...
async def main() -> None:
    ...
    patch_bot_protect_content(bot)
    dp = Dispatcher()

    # Stale callback queries accumulated while the bot was offline must not be
    # replayed: they may mutate data and Telegram refuses to answer them anyway.
    drop_pending_updates = os.getenv("DROP_PENDING_UPDATES", "1").strip().lower() in {
        "1",
        "true",
        "yes",
        "on",
    ...
    tasks: List[asyncio.Task] = [
        asyncio.create_task(auction_publisher_loop(bot)),
        asyncio.create_task(auction_notifications_loop(bot, AUCTION_CHANNEL_USERNAME)),
        asyncio.create_task(auction_winner_loop(bot, DISCUSSION_CHAT_ID)),
        asyncio.create_task(card_subscriptions_watch_loop(bot)),
