...
def test_python_313_is_the_only_supported_runtime() -> None:
    ...
    assert (ROOT / "Dockerfile").read_text(encoding="utf-8").startswith("FROM python:3.13-slim")
    assert (
        (ROOT / "Dockerfile.server-supervisor-proxy")
    ...
    for index, job_name in enumerate(ordered_jobs):
        start = ci.index(f"  {job_name}:")
            ...
            ci.index(f"  {ordered_jobs[index + 1]}:") if index + 1 < len(ordered_jobs) else len(ci)
        )
        job = ci[start:end]
