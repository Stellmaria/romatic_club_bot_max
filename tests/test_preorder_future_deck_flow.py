...
def test_preorder_supports_whole_deck_as_an_alternative_mode() -> None:
    ...
    expected = "Предзаказ: Колода №29 «Будущая история», целая колода"
...
def test_old_preorder_drafts_default_to_items_mode() -> None:
    default_mode = normalize_preorder_mode(None)
    unknown_mode = normalize_preorder_mode("unknown")
    whole_mode = normalize_preorder_mode(PREORDER_MODE_WHOLE_DECK)

    assert default_mode == PREORDER_MODE_ITEMS
    assert unknown_mode == PREORDER_MODE_ITEMS
    assert whole_mode == PREORDER_MODE_WHOLE_DECK


def test_preorder_normalization_drops_invalid_and_zero_values() -> None:
    items = normalize_preorder_items(
        {
            "bronze": "2",
            "silver": 0,
            "gold": "invalid",
...
def test_preorder_whole_deck_requires_explicit_clear_confirmation() -> None:
    handler_path = ROOT / "bot/handlers/auction/preorder.py"
