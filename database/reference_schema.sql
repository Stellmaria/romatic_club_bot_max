create type obtain_type as enum ('diamonds', 'tea');

alter type obtain_type owner to postgres;

create type deck_type as enum ('resource', 'roulette');

alter type deck_type owner to postgres;

-- Unknown how to generate base type type

alter type gtrgm owner to postgres;

create type market_currency as enum ('cups', 'diamonds', 'treasures', 'cash');

alter type market_currency owner to postgres;

create type listing_status as enum ('active', 'hidden', 'sold', 'archived', 'deleted');

alter type listing_status owner to postgres;

create type offer_kind as enum ('cards', 'cups', 'diamonds', 'treasures', 'whole_deck', 'service');

alter type offer_kind owner to postgres;
    ...
    constraint chk_auctions_time_order
        check (
            CASE
                ...
                ELSE true
                END),
    constraint auctions_end_eq_start_plus_31_window
...
create unique index ux_auctions_discussion_msg
    on auctions (discussion_message_id)
    where (discussion_message_id IS NOT NULL);

create unique index ux_auctions_message_id
...
create index idx_auctions_status_starttime
    on auctions (status, start_time);

create index idx_auctions_start_time
    on auctions (start_time);

create index idx_auctions_card_name_trgm
    on auctions using gin (card_name gin_trgm_ops);

create index idx_auctions_hero_name_trgm
    on auctions using gin (hero_name gin_trgm_ops);

create index idx_auctions_sched_by_start
    on auctions (start_time)
    where ((status)::text = 'scheduled'::text);

create index idx_auctions_start_time_date
    on auctions ((start_time::date));

create index idx_auctions_status_names
...
create index idx_auctions_kind
    on auctions (auction_kind);

create index idx_auctions_card_id
    on auctions (card_id);

create index idx_cards_hero_trgm
    on cards using gin (hero_name gin_trgm_ops);

create index idx_cards_name_trgm
    on cards using gin (card_name gin_trgm_ops);

create index idx_cards_hero
    on cards (hero_name);

create index idx_cards_name
    on cards (card_name);

create index idx_cards_deck_num
    on cards (deck_id, num);

create table trusted_usernames
(
    username varchar(32) not null
        primary key
);

alter table trusted_usernames
    owner to postgres;
...
alter table auction_owners
    owner to postgres;

create index idx_auction_owners_auction_id
    on auction_owners (auction_id);

create index idx_auction_owners_user_id
    on auction_owners (user_id);

create index idx_auction_owners_user_folder
...
create index idx_audit_auction_time
...
create index idx_bids_auction_sort
    on bids (auction_id asc, amount desc, placed_at asc);

create index idx_bids_discussion_msg
...
create index idx_user_subscriptions_card
    on user_subscriptions (card_id);

create index idx_user_subs_last_confirmed
...
create index idx_users_is_luxury
    on users (is_luxury);

create table card_day_notifications
(
    id      bigserial
        primary key,
    user_id bigint                  not null,
...
alter table card_day_notifications
    owner to postgres;

create table unreachable_users
(
    user_id   bigint                  not null
        primary key,
...
alter table preset_aliases
    owner to postgres;

create table user_preset_subscriptions
(
    id         bigserial
        primary key,
    user_id    bigint                                 not null,
...
alter table user_preset_subscriptions
    owner to postgres;
...
create index idx_market_listings_currency
    on market_listings (currency_type, cash_code);

create index idx_market_listings_status_updated
    on market_listings (status asc, updated_at desc);

create index idx_market_listings_kind
    on market_listings (offer_kind);

create table market_listing_items
(
    id            serial
        primary key,
...
alter table market_listing_items
    owner to postgres;

create index idx_market_listing_items_listing
    on market_listing_items (listing_id);

create index idx_market_listing_items_card
    on market_listing_items (card_id);

create unique index uq_market_listing_items
    on market_listing_items (listing_id, card_id);

create table market_rate_tiers
(
    id         serial
        primary key,
    listing_id integer           not null
...
alter table market_rate_tiers
    owner to postgres;

create index idx_mrt_listing
    on market_rate_tiers (listing_id, sort_order);

create index idx_market_rate_tiers_listing
...
alter table exchange_batches
    owner to postgres;

create index idx_exchange_batches_status
    on exchange_batches (status);

create index idx_exchange_batches_deck
    on exchange_batches (deck_id);

create index idx_exchange_batches_status_created
...
create index idx_exchange_items_batch
    on exchange_items (batch_id);

create unique index ux_exchange_items_batch_card
    on exchange_items (batch_id, card_id)
    where (card_id IS NOT NULL);

create table guides_thanks
(
    user_id      bigint              not null
        primary key,
...
alter table guides_thanks
    owner to postgres;

create table admin_thanks_totals
(
...
alter table admin_thanks_totals
    owner to postgres;

create table admin_thanks_users
(
    author       text                not null,
    user_id      bigint              not null,
    created_at   timestamp default CURRENT_TIMESTAMP,
...
alter table admin_thanks_users
    owner to postgres;

create table auction_win_mailings
(
    id               bigserial
        primary key,
    auction_id       integer not null,
...
alter table auction_win_mailings
    owner to postgres;

create index idx_auction_win_mailings_auction_id
    on auction_win_mailings (auction_id);

create table auction_manual_results
(
    auction_id        integer not null
        primary key,
...
alter table auction_manual_results
    owner to postgres;

create table auctions_image_id_backup
(
    auction_id   integer not null
        primary key,
    old_image_id text,
...
alter table auctions_image_id_backup
    owner to postgres;

create table auction_posts_backfill
(
...
alter table auction_posts_stats
    owner to postgres;

create index idx_aps_checked
    on auction_posts_stats (checked);

create index idx_aps_excluded
    on auction_posts_stats (excluded);

create index idx_aps_deck_no
    on auction_posts_stats (deck_no);

create index idx_aps_card_title
    on auction_posts_stats (card_title);

create table exchange_print_stats
(
...
alter table exchange_print_stats
    owner to postgres;

create index idx_exchange_print_stats_updated_at
...
alter table autobids
    owner to postgres;

create table autobid_actions
(
    action_id             serial
        primary key,
...
alter table autobid_actions
    owner to postgres;

create index autobid_actions_auction_id_idx
...
alter table user_uids
    owner to postgres;

create unique index ux_user_uids_uid_hash
    on user_uids (uid_hash);

create table uid_verification_requests
(
    id                     bigserial
        primary key,
    user_id                bigint                                           not null
...
alter table uid_verification_requests
    owner to postgres;

create index idx_uid_verif_req_status_created
    on uid_verification_requests (status asc, created_at desc);

create index idx_uid_verif_req_uid
    on uid_verification_requests (uid);

create index idx_uid_verif_req_uid_hash
...
create index idx_uid_bans_until
    on uid_bans (banned_until);

create unique index ux_uid_bans_uid_hash
    on uid_bans (uid_hash);

create table uid_verification_confirmations
(
    id                    bigserial
        primary key,
...
alter table uid_verification_confirmations
    owner to postgres;

create table admin_thanks_clicks
(
...
alter table admin_thanks_clicks
    owner to postgres;

create table uid_verification_events
(
    id             bigserial
        primary key,
    request_id     bigint                                       not null
...
alter table uid_verification_events
    owner to postgres;

create index idx_uidv_events_req_time
    on uid_verification_events (request_id asc, created_at desc);

create table uid_verification_confirmation_reminders
(
...
alter table uid_verification_confirmation_reminders
    owner to postgres;

create view v_user_uid_status
            (tg_user_id, tg_username, uid, uid_status, verified_at, verified_by, uid_is_banned, banned_until,
             ban_reason) as
SELECT u.user_id                                                                   AS tg_user_id,
       ...
       uu.verified_at,
       uu.verified_by,
       ub.uid IS NOT NULL AND (ub.banned_until IS NULL OR ub.banned_until > now()) AS uid_is_banned,
       ub.banned_until,
       ub.reason                                                                   AS ban_reason
FROM users u
         LEFT JOIN user_uids uu ON uu.user_id = u.user_id
         LEFT JOIN uid_bans ub ON ub.uid = uu.uid;

alter table v_user_uid_status
    owner to postgres;
...
create function is_valid_bid(p_auction_id integer, p_amount integer) returns boolean
    stable
    language plpgsql
as
$$
...
alter function is_valid_bid(integer, integer) owner to postgres;

create function prevent_currency_change_if_bids() returns trigger
    language plpgsql
as
$$
BEGIN
  IF NEW.currency IS DISTINCT FROM OLD.currency
     AND EXISTS (SELECT 1 FROM public.bids b WHERE b.auction_id = OLD.auction_id)
  THEN RAISE EXCEPTION 'Нельзя менять валюту после появления ставок'; END IF;
  RETURN NEW;
END $$;

alter function prevent_currency_change_if_bids() owner to postgres;
...
create function prevent_time_change_if_bids() returns trigger
    language plpgsql
as
$$
BEGIN
  IF (NEW.start_time IS DISTINCT FROM OLD.start_time OR NEW.end_time IS DISTINCT FROM OLD.end_time)
     AND EXISTS (SELECT 1 FROM public.bids b WHERE b.auction_id = OLD.auction_id)
  THEN RAISE EXCEPTION 'Нельзя менять время аукциона после появления ставок'; END IF;
  RETURN NEW;
END $$;

alter function prevent_time_change_if_bids() owner to postgres;

create function list_missing_ids(_schema text, _table text, _pk text, _limit integer DEFAULT 1000)
...
BEGIN
  RETURN QUERY EXECUTE format($q$
    ...
    ORDER BY missing_id
    LIMIT $1
  $q$, _pk, _schema, _table)
  USING _limit;
END;
$$;

alter function list_missing_ids(text, text, text, integer) owner to postgres;

create function touch_market_listing() returns trigger
    language plpgsql
as
$$
BEGIN
...
alter function touch_market_listing() owner to postgres;
...
create function auctions_fix_end_time() returns trigger
    language plpgsql
as
$$
BEGIN
...
alter function auctions_fix_end_time() owner to postgres;

create function norm_username(t text) returns text
    immutable
    language sql
as
$$
...
alter function norm_username(text) owner to postgres;

create function norm_hero(t text) returns text
    immutable
    language sql
as
$$
...
alter function norm_hero(text) owner to postgres;

create function uid_verif_sync_cols() returns trigger
    language plpgsql
as
$$
BEGIN
  ...
  -- профиль: туда-сюда
  NEW.profile_proof_file_id := COALESCE(NEW.profile_proof_file_id, NEW.profile_file_id);
  NEW.profile_file_id       := COALESCE(NEW.profile_file_id, NEW.profile_proof_file_id);

  RETURN NEW;
END;
$$;

alter function uid_verif_sync_cols() owner to postgres;
