...
Единственный исполняемый runner находится в `db/migrator.py`. Он читает только
`db/migrations/NNN_*.sql`, проверяет формат и уникальность версий, сверяет
SHA-256 уже применённых файлов и сериализует запуск replicas через PostgreSQL
advisory lock. Любое изменение схемы оформляется новым номером в этом каталоге.

`db/migrations.py` является только совместимым фасадом для старых импортов и
делегирует в `db.migrator`; собственного lock, журнала и каталога SQL у него нет.
Файлы `database/migrations/*.sql` сохранены исключительно как архив прежней
истории, не упаковываются и не выполняются. `database/bootstrap.sql`
