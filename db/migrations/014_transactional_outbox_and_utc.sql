-- Canonical continuation: durable Telegram delivery and timezone-safe timestamps.
--
-- Historical timestamp-without-time-zone values were written as
-- Europe/Moscow wall-clock time. Preserve their instants while moving the
-- runtime schema to timestamptz. All guards are intentionally idempotent so
-- installations that received the former root-level migration remain safe.
