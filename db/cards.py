"""Card, deck and card-economy queries.
...
async def set_card_video_by_id(
    ...
    Временные поля лота не меняются. Миграция 004 удалила исторический
    CHECK на фиксированные 31 минуту, поэтому медиа можно синхронизировать
    для всех совпавших лотов независимо от их длительности.
    """
    ...
    async with db_pool.acquire() as conn:
        ...
        try:
            auctions_updated_strict = await _update_auctions_strict("")
            auctions_updated_fallback = await _update_auctions_fallback("")
        except Exception as exc:
            auctions_error = str(exc).splitlines()[0]
