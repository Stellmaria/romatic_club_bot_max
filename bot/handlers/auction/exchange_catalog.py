...
from bot.handlers.auction.exchange_moderation import fmt_dt_msk, user_link
...
async def _q_exchange_approved_decks() -> list[dict]:
    ...
    """
    exchange_deck_ids = await get_exchange_deck_ids()

    queries = await ExchangeCatalogQueries.create()
    rows = await queries.approved_deck_counts(exchange_deck_ids)
...
async def _q_exchange_approved_batches_by_card(deck_id: int, card_id: int) -> list[int]:
    ...
    queries = await ExchangeCatalogQueries.create()
    return await queries.approved_batch_ids_by_card(
        status=EX_STATUS_APPROVED,
        deck_id=deck_id,
        card_id=card_id,
        modes=EX_MODE_CARDLIKE,
    )
...
async def _q_exchange_has_whole_deck_lot(deck_id: int) -> bool:
    queries = await ExchangeCatalogQueries.create()
    return await queries.has_whole_deck_lot(
        status=EX_STATUS_APPROVED,
        mode=EX_MODE_DECK,
        deck_id=deck_id,
    )
...
async def _q_exchange_whole_deck_count(deck_id: int) -> int:
    queries = await ExchangeCatalogQueries.create()
    return await queries.whole_deck_count(
        status=EX_STATUS_APPROVED,
        deck_id=deck_id,
        modes=EX_WHOLE_DECK_MODES,
    )
...
async def _q_exchange_approved_whole_deck_batch_ids(deck_id: int) -> list[int]:
    queries = await ExchangeCatalogQueries.create()
    return await queries.approved_whole_deck_batch_ids(
        deck_id=deck_id,
        mode=EX_MODE_DECK,
    )
...
async def _q_exchange_approved_cards_by_deck(deck_id: int) -> list[dict]:
    queries = await ExchangeCatalogQueries.create()
    return await queries.approved_cards_by_deck(
        status=EX_STATUS_APPROVED,
        deck_id=deck_id,
        modes=EX_MODE_CARDLIKE,
    )
...
async def _q_exchange_card_batches(deck_id: int, card_id: int, limit: int = 80) -> list[dict]:
    queries = await ExchangeCatalogQueries.create()
    return await queries.card_batches(
        status=EX_STATUS_APPROVED,
        deck_id=deck_id,
        card_id=card_id,
        modes=EX_MODE_CARDLIKE,
...
async def ex_appr_list_all(call: types.CallbackQuery):
    ...
    queries = await ExchangeCatalogQueries.create()
    rows = await queries.approved_lots()
...
async def _q_exchange_whole_deck_batches(deck_id: int, limit: int = 50) -> list[dict]:
    queries = await ExchangeCatalogQueries.create()
    return await queries.whole_deck_batches(
        status=EX_STATUS_APPROVED,
        deck_id=deck_id,
        modes=EX_WHOLE_DECK_MODES,
        limit=limit,
...
async def _q_exchange_deck_total_cards(deck_id: int) -> int:
    queries = await ExchangeCatalogQueries.create()
    return await queries.deck_total_cards(deck_id)


async def _q_exchange_batch_items_count(batch_id: int) -> int:
    queries = await ExchangeCatalogQueries.create()
    return await queries.batch_items_count(batch_id)
...
async def _q_exchange_decks_with_approved() -> list[int]:
    exchange_deck_ids = await get_exchange_deck_ids()
    queries = await ExchangeCatalogQueries.create()
    return await queries.decks_with_approved(
        status=EX_STATUS_APPROVED,
        deck_ids=exchange_deck_ids,
    )
