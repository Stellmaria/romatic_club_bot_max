...
from bot.handlers.admin.services.market_keyboards import edit_listing_kb, my_listing_actions, listing_public_kb, \
    market_reply_kb
from bot.services.market_legacy.market_render import _reload_listing_inplace
from bot.services.market_legacy.market_utils import ensure_owner, can_bump_now, _upsert_price, safe_delete
...
async def ms_f_active(message: Message, state: FSMContext):
    await importlib.import_module("bot.services.market_legacy.market_add_flow")._my_sales_set_filter_and_show(message, state, "active")


@router.message(F.chat.type == "private", F.text.regexp(r"^(?:[▪▫]\s)?Скрытые$"))
async def ms_f_hidden(message: Message, state: FSMContext):
    await importlib.import_module("bot.services.market_legacy.market_add_flow")._my_sales_set_filter_and_show(message, state, "hidden")


@router.message(F.chat.type == "private", F.text.regexp(r"^(?:[▪▫]\s)?Проданные$"))
async def ms_f_sold(message: Message, state: FSMContext):
    await importlib.import_module("bot.services.market_legacy.market_add_flow")._my_sales_set_filter_and_show(message, state, "sold")


@router.message(F.chat.type == "private", F.text.regexp(r"^(?:[▪▫]\s)?Архив$"))
async def ms_f_archived(message: Message, state: FSMContext):
    await importlib.import_module("bot.services.market_legacy.market_add_flow")._my_sales_set_filter_and_show(message, state, "archived")


@router.message(F.chat.type == "private", F.text.regexp(r"^(?:[▪▫]\s)?Все$"))
async def ms_f_all(message: Message, state: FSMContext):
    await importlib.import_module("bot.services.market_legacy.market_add_flow")._my_sales_set_filter_and_show(message, state, "all")
